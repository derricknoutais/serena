<template>
    <AppLayout title="Modifier le bon d'achat">
        <section class="space-y-6">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-serena-text-main">Modifier le bon d'achat</h1>
                    <p class="text-sm text-serena-text-muted">
                        Mettez à jour les informations avant la réception du stock.
                    </p>
                </div>
                ...
                <div class="flex flex-col gap-2 lg:items-end">
                    ...
                    <div class="flex gap-2">
                        <button
                            ...
                            @click="$inertia.visit(`/stock/purchases/${purchase.id}`)"
                        >
                            Annuler
                        </button>
                        <PrimaryButton
                        ...
                        >
                            {{ purchaseSubmitting ? 'Mise à jour…' : 'Mettre à jour' }}
                        </PrimaryButton>
                    </div>
                </div>
            </form>
...
export default {
        ...
        stockItems: {
            handler(items) {
                if (!this.form.lines?.length && items.length) {
                    this.form.lines = [this.emptyLine(this.form.currency || 'XAF')];
                }
            },
            immediate: true,
        },
    ...
    methods: {
        buildForm() {
            const currency = this.purchase.currency || 'XAF';
            const lines = this.purchase.lines?.length
                ? this.purchase.lines.map((line) => ({
                ...
                : [this.emptyLine(currency)];

            return {
                storage_location_id: this.purchase.storage_location?.id ?? '',
                reference_no: this.purchase.reference_no ?? '',
                supplier_name: this.purchase.supplier_name ?? '',
                purchased_at: this.purchase.purchased_at ?? '',
                currency,
                lines,
            };
        },
        emptyLine(currency = 'XAF') {
            return {
                stock_item_id: '',
                quantity: 1,
                unit_cost: 0,
        ...
        async submitPurchase() {
            ...
            try {
                await axios.put(`/stock/purchases/${this.purchase.id}`, this.form);

                Swal.fire({
                    icon: 'success',
                    title: 'Bon d’achat mis à jour',
                    timer: 1400,
                    showConfirmButton: false,
                });
                this.$inertia.visit(`/stock/purchases/${this.purchase.id}`);
            } catch (error) {
                Swal.fire({
                    icon: 'error',
                    title: 'Erreur',
                    text: error.response?.data?.message ?? 'Impossible de mettre à jour le bon.',
                });
            } finally {
